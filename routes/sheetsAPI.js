...
function scrapPublication(callback) {
    sheets.spreadsheets.values.batchGet({
        spreadsheetId: sheetId,
        ranges: ["'ผลงานวิจัย'"],
        auth: clientAuth
...
function scrapResearchFundSheet(callback) {
    sheets.spreadsheets.values.batchGet({
        spreadsheetId: sheetId,
        ranges: ["'ทุนวิจัย'"],
        auth: clientAuth
...
function scrapIntellectualProperty(callback) {
    sheets.spreadsheets.values.batchGet({
        spreadsheetId: sheetId,
...
function scrapTeacherTraining(callback) {
    sheets.spreadsheets.values.batchGet({
        spreadsheetId: sheetId,
...
router.get('/getDataSetup', async function(req, res) {
    ...
    if (response1.code == '999999' && response1.data.length > 0) {
        ...
        for (let i = 0; i < depts_new.length; i++) {
            ...
            for (let j = 0; j < deptData.length; j++)
                if (depts_new[i]._id == deptData[j]._id) { isNew = false; break; }
    ...
    if (response2.code == '999999' && response2.data.length > 0) {
        ...
        for (let i = 0; i < pos_new.length; i++) {
            ...
            for (let j = 0; j < positionData.length; j++)
                if (pos_new[i]._id == positionData[j]._id) { isNew = false; break; }
    ...
    if (response3.code == '999999' && response3.data.length > 0) {
        ...
        for (let i = 0; i < acads_new.length; i++) {
            let isNew = true;
            for (let j = 0; j < academicData.length; j++)
                if (acads_new[i]._id == academicData[j]._id) { isNew = false; break; }
